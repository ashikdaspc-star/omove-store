...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-3 sm:px-5 md:px-6 lg:px-8 py-4 sm:py-8 space-y-4 sm:space-y-8 font-sans">
      {/* Header Profile Card - Highlighted Dark Slate Theme */}
      <div className="p-4 sm:p-8 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#064E3B] via-[#04392b] to-slate-900 text-white shadow-xl border border-emerald-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6 relative overflow-hidden">
        <div className="flex items-center gap-3.5 sm:gap-5 w-full md:w-auto">
          <div className="w-12 h-12 sm:w-20 sm:h-20 rounded-full bg-emerald-500 text-slate-950 flex items-center justify-center font-bold text-xl sm:text-3xl shadow-lg shadow-emerald-500/20 font-mono shrink-0">
            {customerProfile.name ? customerProfile.name.charAt(0).toUpperCase() : 'C'}
          </div>

          <div className="space-y-0.5 sm:space-y-1 min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h1 className="text-lg sm:text-3xl font-extrabold text-white truncate">{customerProfile.name || 'Customer'}</h1>
              <span className="px-2 py-0.5 rounded text-[9px] sm:text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-400/40 font-mono shrink-0">
                VERIFIED
              </span>
            </div>

            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] sm:text-xs text-emerald-100/80 font-mono">
              <span className="flex items-center gap-1 truncate max-w-[180px] sm:max-w-none">
                <Mail className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400 shrink-0" />
                <span className="truncate">{customerProfile.email || 'customer@omove.tech'}</span>
              </span>
        ...
        <button
          ...
          className="w-full md:w-auto px-4 py-2 sm:px-5 sm:py-3 rounded-xl sm:rounded-2xl bg-white/10 hover:bg-white/20 text-white font-mono text-xs font-bold border border-white/20 flex items-center justify-center gap-2 transition-all cursor-pointer"
        >
      ...
      {showDeleteAccountModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm font-sans">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-6 space-y-5 shadow-2xl text-slate-900 animate-fadeIn">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-rose-600 font-bold text-sm">
                <AlertTriangle className="w-5 h-5" />
                <span>Confirm Account Deletion</span>
              </div>
              <button onClick={() => setShowDeleteAccountModal(false)} className="p-1.5 rounded-lg bg-slate-100 text-slate-500 hover:text-slate-900 hover:bg-slate-200 transition-colors cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs space-y-2">
              <strong className="block text-rose-900 font-bold text-sm">Warning: This action cannot be undone!</strong>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-xs flex items-center gap-2 transition-colors cursor-pointer"
              >
                {isDeletingAccount ? (
                  <>
