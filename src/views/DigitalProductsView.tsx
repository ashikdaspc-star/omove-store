...
export const DigitalProductsView: React.FC<DigitalProductsViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 font-sans">
      {/* Top Control Bar */}
      <div className="scroll-reveal p-4 sm:p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        {/* Left: Breadcrumbs & Title */}
        <div className="space-y-1">
          <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
            <Link to="/" className="hover:text-slate-900 transition-colors">Home</Link>
            <ChevronRight className="w-3 h-3 text-slate-400" />
            <Link to="/digital-products" className="hover:text-slate-900 transition-colors">Digital</Link>
            {activeCategory && (
              <>
                <ChevronRight className="w-3 h-3 text-slate-400" />
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="relative w-full md:w-56 shrink-0">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 rounded-lg bg-slate-50 border border-slate-200 focus:border-emerald-600 focus:bg-white text-xs text-slate-900 placeholder-slate-400 focus:outline-none transition-colors"
            />
          </div>
        </div>
        ...
        {childSubcategories.length > 0 && (
          <div className="pt-3 border-t border-slate-100 flex items-center gap-1.5 overflow-x-auto text-xs w-full">
            <span className="text-xs text-slate-400 uppercase font-bold shrink-0">Subcategories:</span>
            <button
      ...
      <div className="space-y-3">
        ...
        {filteredProducts.length === 0 ? (
          <div className="scroll-reveal p-10 text-center bg-white rounded-2xl border border-dashed border-slate-200 space-y-3">
