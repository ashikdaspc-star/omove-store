  ...
  return (
    ...
    >
      <div>
        ...
        {/* Content Body */}
        <div className="p-3 sm:p-5 space-y-2 sm:space-y-3 cursor-pointer" onClick={() => onSelect(product)}>
          ...
          <h3 className="font-bold text-sm sm:text-base text-slate-900 group-hover:text-emerald-600 transition-colors line-clamp-1">
            {product.name}
          </h3>
          ...
          {/* Key Feature Bullets */}
          <ul className="space-y-1 text-[10px] sm:text-[11px] text-slate-600 pt-0.5 sm:pt-1">
          ...
          <div className="pt-1 sm:pt-2">
            <span className="text-[9px] sm:text-[10px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200/80 px-2 py-1 rounded-lg flex items-center gap-1.5 leading-tight">
      ...
      {/* Footer Pricing & CTA */}
      <div className="p-3 sm:p-5 pt-2 sm:pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
        <div>
          <div className="flex items-baseline gap-1 sm:gap-1.5">
            <span className="text-base sm:text-lg font-bold font-mono text-slate-900">₹{product.price}</span>
            {product.originalPrice > product.price && (
              <span className="text-[10px] sm:text-xs text-slate-400 line-through font-mono">₹{product.originalPrice}</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          <button
            ...
            className="p-2 sm:p-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors min-h-[36px] min-w-[36px] sm:min-h-[44px] sm:min-w-[44px] flex items-center justify-center cursor-pointer"
            title="Add to Cart"
          >
          ...
          <button
            ...
            className={`px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-[11px] sm:text-xs font-bold font-mono tracking-wide transition-all min-h-[36px] sm:min-h-[44px] flex items-center justify-center whitespace-nowrap cursor-pointer ${
              !isOnline
                ? 'bg-slate-200 text-slate-400 border border-slate-300 cursor-not-allowed shadow-none'
                : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs hover:scale-105 active:scale-95'
            }`}
          >
            {isOnline ? 'BUY NOW' : 'OFFLINE'}
          </button>
        </div>
      </div>
    </div>
